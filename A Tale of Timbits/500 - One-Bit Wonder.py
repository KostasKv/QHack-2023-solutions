...
def calculate_timbit(U, rho_0, rho, n_iters):
    ...
    @qml.qnode(dev)
    def circuit(state, trace_out):
        qml.QubitDensityMatrix(state, wires=range(2))
        retain = 1 if trace_out == 0 else 0
        return qml.density_matrix(wires=[retain])
    
    U = np.matrix(U)

    for i in range(n_iters): 
        state = U @ np.kron(rho_0, rho) @ U.getH()
        rho = circuit(state, trace_out=0)
...
def apply_timbit_gate(U, rho_0, timbit):
    ...
    U = np.matrix(U) 
    state = U @ np.kron(rho_0, timbit) @ U.getH()


    output = circuit(state, trace_out=1)
    return output
...
def SAT(U_f, q, rho, n_bits):
    ...
    # Step 3
    final_qubit_state = circuit()
    for _ in range(q):
        timbit = calculate_timbit(U_NP, final_qubit_state, rho, n_iters=100)
        final_qubit_state = apply_timbit_gate(U_NP, final_qubit_state, timbit)
    
    # Get measurement probabilities
    dev2 = qml.device("default.mixed", wires=1)
    @qml.qnode(dev2)
    def circuit2():
        qml.QubitDensityMatrix(final_qubit_state, wires=[0])
